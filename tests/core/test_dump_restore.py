...
def test_dump(simulation_builder, tax_benefit_system, couple, month):
    directory = tempfile.mkdtemp(prefix = "openfisca_")
    simulation = simulation_builder.build_from_entities(tax_benefit_system, couple)
    calculated_value = simulation.calculate('disposable_income', month)
    ...
    cached_value = disposable_income_holder.get_array(month)
